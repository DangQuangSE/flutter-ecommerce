...
- Use this when adding state management (BLoC or Cubit) to an existing feature.
- Use this to ensure BLoCs are generated using Dart 3 sealed classes and Equatable, avoiding bad state patterns or stale singleton BLoCs.
...
final class {Feature}DetailLoaded extends {Feature}State {
  ...
  const {Feature}DetailLoaded(item);
  @override
